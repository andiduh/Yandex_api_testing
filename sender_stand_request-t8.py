...
def test_1create_user_sym_letter_in_first_name_get_success_response():
    user_body = get_user_body(data.name)
    user_response = sender_stand_request4.post_new_user(user_body, data.headers)
    assert user_response.status_code == 400

def test_2create_user_sym_letter_in_first_name_get_success_response():
    user_body = get_user_body(data.name)
    user_response = sender_stand_request4.post_new_user(user_body, data.headers)
    assert user_response.json()["code"] == 400

def test_3create_user_sym_letter_in_first_name_get_success_response():
    user_body = get_user_body(data.name)
    user_response = sender_stand_request4.post_new_user(user_body, data.headers)
